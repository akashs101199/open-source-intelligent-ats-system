...
const MatchResults = ({ matches, loading, error, processingTime, jobTitle, onRetry }) => {
  ...
  if (error) {
    return <ErrorMessage message={`Matching failed: ${error}`} onRetry={onRetry} />;
  }
  ...
  if (!matches || matches.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400">Upload resumes and select a job to see candidate matches</p>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-2xl font-semibold mb-2">Top Matches {jobTitle && `for ${jobTitle}`}</h2>
      ...
      <div className="space-y-6">
        {matches.map((match, index) => (
          <CandidateScore key={match.candidate_id} match={match} index={index} />
        ))}
      </div>
    </div>
  );
